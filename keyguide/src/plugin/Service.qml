...
Item {
  id: root
  Components.AppLook { id: appLook }
  readonly property string appLanguage: appLook.preferences.preferences && appLook.preferences.preferences.language || ""
  onAppLanguageChanged: Qt.callLater(refreshSettings)
  ...
  property string settingsPath: (Quickshell.env("XDG_CONFIG_HOME") || home + "/.config") + "/bindlume/keyboard-guide.json"
  ...
  readonly property var presentationSettingKeys: [
    ...
    "scale",
    "opacity",
    "fadeDurationMs", "showActionIndicators", "showSavedIndicators", "compactView", "badgeMode", "showAppIcons", "showIcons", "showHiddenItems", "showDelayMs", "compactIcons", "followTheme",
    "groups",
    "hiddenBindingIds",
    "language"
  ]
  ...
  readonly property var displayBindings: VisibilityModel.presentedBindings(
    ...
    settings.language).map(function(binding) {
      const result = Object.assign({}, binding)
      const target = String(binding.targetId || "")
      const origin = target.startsWith("webapp:") ? target.match(/^webapp:https?:\/\/[^/]+/) : null
      result.bookmarked = root.appBookmarks.indexOf(String(binding.app_id || "")) !== -1
      result.appHidden = root.appHiddenIds.indexOf(String(binding.app_id || "")) !== -1
      result.icon = root.iconAssets[target] || (origin ? root.iconAssets[origin[0]] : "") || result.icon
      return result
    })
  property bool globalGuideOpen: false
  readonly property var globalBindings: displayBindings.filter(function(binding) {
    return (binding.modifiers || []).indexOf("SUPER") === -1
      && (settings.hiddenBindingIds || []).indexOf(binding.presentation_id || binding.id) === -1
  })
  readonly property var bindings: (globalGuideOpen ? globalBindings : HudModel.forGroup(
    displayBindings,
    modifiers,
    settings.hiddenBindingIds
  )).filter(function(binding) { return settings.showHiddenItems === true || !binding.appHidden })
  property bool keyboardViewActive: false
  Timer {
    id: keyboardViewLease
    interval: 10000
    onTriggered: root.keyboardViewActive = false
  }
  readonly property bool hudEligible: !keyboardViewActive && HudModel.shouldShow({
  ...
  readonly property bool hudVisible: !locked && !keyboardViewActive && (globalGuideOpen || revealDelay.ready)
  Components.RevealDelay {
    id: revealDelay
    eligible: root.hudEligible
    delayMs: Number(root.settings.showDelayMs || 0)
  }
  ...
  function initializeRuntime() {
    ...
    const serviceUrl = String(Qt.resolvedUrl("Service.qml"))
    const sourceRoot = repositoryPluginRoot() || (serviceUrl.startsWith("file://")
      && serviceUrl.endsWith("/src/plugin/Service.qml")
      ? decodeURIComponent(serviceUrl.slice(7, -"/src/plugin/Service.qml".length)) : "")
    if (!sourceRoot) {
      runtimeReady = true
      refresh()
      return
  ...
  function refresh() {
    if (runtimeReady && !iconsProcess.running) iconsProcess.running = true
    refreshBindings()
    refreshSettings()
  ...
  function parseBindings(value) {
    ...
    return parsed.map(function(binding) {
      ...
      return {
        app_id: appId, id: binding.id, modifiers: modifiers, key: binding.key,
  ...
  function parseSettings(value) {
    const parsed = typeof value === "string"
      ? JSON.parse(String(value || "")) : value
    if (parsed.showSavedIndicators === undefined) parsed.showSavedIndicators = false
    if (parsed.compactView === undefined) parsed.compactView = parsed.compactIcons === true
    if (parsed.badgeMode === undefined) parsed.badgeMode = parsed.compactIcons === true ? "icons" : "badges"
    if (parsed.showAppIcons === undefined) parsed.showAppIcons = true
    if (parsed.showIcons === undefined) parsed.showIcons = true
    if (parsed.showHiddenItems === undefined) parsed.showHiddenItems = false
    const names = [
      "version", "enabled", "position", "scale", "opacity", "groups",
      "hiddenBindingIds", "fadeDurationMs", "showActionIndicators", "showSavedIndicators", "compactView", "badgeMode", "showAppIcons", "showIcons", "showHiddenItems", "showDelayMs", "compactIcons", "followTheme", "language"
    ]
    if (!hasExactKeys(parsed, names) || parsed.version !== 2
        ...
        || ["badges", "icons", "none"].indexOf(parsed.badgeMode) === -1
        || ["compactView", "showAppIcons", "showIcons", "showHiddenItems"].some(function(key) { return typeof parsed[key] !== "boolean" })
        || typeof parsed.showSavedIndicators !== "boolean"
        || typeof parsed.showActionIndicators !== "boolean"
        || !Number.isInteger(parsed.fadeDurationMs) || parsed.fadeDurationMs < 0 || parsed.fadeDurationMs > 2000
        || !Number.isInteger(parsed.showDelayMs) || parsed.showDelayMs < 0 || parsed.showDelayMs > 5000
        || typeof parsed.compactIcons !== "boolean"
        || typeof parsed.followTheme !== "boolean"
        || ["en", "ko", "ja", "zh_CN", "es", "de", "fr", "uk", "hu"].indexOf(parsed.language) === -1) {
    ...
    return {
      ...
      hiddenBindingIds: parsed.hiddenBindingIds.slice(), followTheme: parsed.followTheme, compactIcons: parsed.compactIcons, showDelayMs: parsed.showDelayMs, fadeDurationMs: parsed.fadeDurationMs, showActionIndicators: parsed.showActionIndicators, showSavedIndicators: parsed.showSavedIndicators,
      compactView: parsed.compactView, badgeMode: parsed.badgeMode, showAppIcons: parsed.showAppIcons, showIcons: parsed.showIcons, showHiddenItems: parsed.showHiddenItems,
      language: root.appLanguage || parsed.language
    }
  }
  ...
  function hudState() {
    return {
      visible: hudVisible,
      keyboardViewActive: keyboardViewActive,
      modifiers: modifiers,
      bindings: bindings
    }
  }
  ...
  function configureHud() {
    ...
    hudLoader.item.globalMode = Qt.binding(function() { return root.globalGuideOpen })
    hudLoader.item.sessionLocked = Qt.binding(function() { return root.locked })
    hudLoader.item.incomingBindings = Qt.binding(function() { return root.bindings })
  ...
  onLockedChanged: if (locked) { globalGuideOpen = false; clearModifierState() }
  ...
  Process {
    id: iconsProcess
    command: ["python3", "-m", "keyguide_backend", "icons"]
    environment: root.backendEnvironment
    stdout: StdioCollector { id: iconsOutput; waitForEnd: true }
    onExited: function(code) {
      if (code === 0) {
        try { root.iconAssets = JSON.parse(iconsOutput.text) } catch (error) { console.warn("Guide icons:", error) }
      }
    }
  }
  ...
  property var appBookmarks: []
  property var appHiddenIds: []
  readonly property string appConfig: (Quickshell.env("XDG_CONFIG_HOME") || home + "/.config") + "/bindlume/"
  FileView {
    id: bookmarksFile
    path: root.appConfig + "favorites.json"
    watchChanges: true
    printErrors: false
    onFileChanged: reload()
    onLoaded: {
      try { const values = JSON.parse(text()); root.appBookmarks = Array.isArray(values) ? values : [] }
      catch (error) { root.appBookmarks = [] }
    }
  }
  FileView {
    id: hiddenFile
    path: root.appConfig + "learned.json"
    watchChanges: true
    printErrors: false
    onFileChanged: reload()
    onLoaded: {
      try { const values = JSON.parse(text()); root.appHiddenIds = Array.isArray(values) ? values : [] }
      catch (error) { root.appHiddenIds = [] }
    }
  }
  ...
  IpcHandler {
    ...
    function keyboardView(value: string): string {
      const active = value === "true"
      root.keyboardViewActive = active
      if (active) keyboardViewLease.restart()
      else keyboardViewLease.stop()
      return "ok"
    }

    function toggleGlobal(): string {
      if (root.locked) return "locked"
      root.globalGuideOpen = !root.globalGuideOpen
      if (root.globalGuideOpen) root.refreshBindings()
      return root.globalGuideOpen ? "shown" : "hidden"
    }
    ...
    function presentations(): string {
      return JSON.stringify({bindings: root.displayBindings.map(function(b) {
        return {key:b.key, modifiers:b.modifiers, description:b.description, displayKind:b.displayKind, icon:b.icon, app_id:b.app_id, bookmarked:b.bookmarked, appHidden:b.appHidden}
      }), showActions: root.settings.showActionIndicators !== false})
    }
