...
Item {
  ...
  readonly property var visibleBindings: root.previewGroupEnabled ? HudModel.forGroup(root.bindingsForModel(root.bindings), root.arrayFrom(root.previewModifiers), root.arrayFrom(root.settings.hiddenBindingIds)).filter(function(binding) { return root.settings.showHiddenItems === true || !binding.appHidden }) : []
  ...
  readonly property real previewMargin: root.compactIcons ? 8 : 16
  readonly property real previewTextHeight: 30
  readonly property real previewRowHeight: root.compactIcons ? 32 : previewTextHeight + 8
  readonly property real previewRowsHeight: previewList.count > 0
    ...
    : 44
  readonly property real previewNaturalCardHeight: 16 + 26 + 8 + previewRowsHeight + 12 + (root.badgeMode === "icons" ? legend.height + 12 : 0)
  ...
  Rectangle {
    ...
    Item {
      ...
      Rectangle {
        ...
        width: Math.min(parent.width * 0.92, root.compactIcons ? 520 : 430, Math.max(1, (parent.width - root.previewMargin * 2) / root.previewScale))
        ...
        anchors.topMargin: 8
        anchors.bottomMargin: root.badgeMode === "icons" ? legend.height + 24 : 12
        ...
        delegate: Item {
          ...
          Rectangle {
            ...
            width: Math.max(0, Math.min(root.compactIcons ? 70 : 94, parent.width * 0.48))
          ...
          Image {
            ...
            visible: String(source || "") !== ""
              && (root.showActions || String(modelData.displayKind || "action") !== "action")
              && (["desktopApp", "webapp"].indexOf(String(modelData.displayKind || "")) !== -1 ? root.showAppIcons : root.showIcons)
          ...
          Rectangle {
            ...
            visible: root.badgeMode === "badges" && parent.width >= 180 && (displayKind !== "action" || root.showActions)
            anchors.right: parent.right
            anchors.verticalCenter: parent.verticalCenter
          ...
          Text {
            id: typeIcon
            readonly property string displayKind: String(modelData.displayKind || "action")
            objectName: "hudPreviewTypeIcon-" + String(modelData.id || "")
            visible: root.badgeMode === "icons" && (displayKind !== "action" || root.showActions)
            anchors.right: parent.right
            anchors.verticalCenter: parent.verticalCenter
            width: visible ? 20 : 0
            text: VisibilityModel.categoryIcon(displayKind)
            color: VisibilityModel.typeAccent(displayKind, root.surfaceIsLight(), root.previewAccent)
            font.family: root.fontFamily
            font.pixelSize: 14
            horizontalAlignment: Text.AlignHCenter
          }
          ...
          Text {
            ...
            width: Math.max(0, (typeBadge.visible ? typeBadge.x - 6
              : typeIcon.visible ? typeIcon.x - 6 : parent.width) - x)
            height: root.previewTextHeight
            text: (root.settings.showSavedIndicators === true
              ? (modelData.bookmarked ? "★ " : "") + (modelData.appHidden ? "󰈉 " : "") : "")
              + String(modelData.description || "")
            color: root.previewForeground
            font.family: root.fontFamily
            font.pixelSize: 12
            verticalAlignment: Text.AlignVCenter
            wrapMode: root.compactIcons ? Text.NoWrap : Text.WordWrap
            maximumLineCount: root.compactIcons ? 1 : 2
            elide: Text.ElideRight
          }
        }
        ...
        Flow {
          ...
          spacing: 8
          Repeater {
            model: VisibilityModel.categoryLegendKinds(root.showActions)
            delegate: Text {
              required property string modelData
              text: VisibilityModel.categoryIcon(modelData) + " " + I18n.text(root.language, VisibilityModel.typeBadgeKey(modelData), {})
              color: VisibilityModel.typeAccent(modelData, root.surfaceIsLight(), root.previewAccent)
              font.family: root.fontFamily
              font.pixelSize: 9
            }
          }
